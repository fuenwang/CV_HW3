...
for i = 1:num_categories
    ...
    %is bad
    true_positives = test_image_paths(strcmp(categories{i}, test_labels) & strcmp(categories{i}, predicted_categories));
    %size(test_labels)
    %size(predicted_categories)
    %error('GG')
    %true_positives = strcmp(categories{i}, test_labels) & strcmp(categories{i}, predicted_categories);
    %False positives. There might not be enough of them if the classifier
    %is good
    false_positive_inds = ~strcmp(categories{i}, test_labels) & strcmp(categories{i}, predicted_categories);
    ...
    %is good
    false_negative_inds = strcmp(categories{i}, test_labels) & strcmp(categories{i}, predicted_categories);
